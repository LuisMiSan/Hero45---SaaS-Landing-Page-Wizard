...
`;
  ...
  const handleDownloadPDF = () => {
    ...
    const archList = project.architecture.map(id => {
       const comp = COMPONENTS.find(c => c.id === id);
       return comp 
         ? `<div class="module-card"><span class="icon">${comp.icon}</span> ${comp.name}</div>` 
         : '';
    }).join('');
    ...
    const content = `
      ...
      <html>
      <head>
        <title>Hero45 Blueprint - ${project.title}</title>
        <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;900&family=Fira+Code&family=Material+Symbols+Outlined" rel="stylesheet"/>
        <style>
          @page { margin: 0; size: A4; }
          body { font-family: 'Inter', sans-serif; padding: 40px; max-width: 800px; margin: 0 auto; color: #1a1a1a; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          
          .header { display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 2px solid #000; padding-bottom: 20px; margin-bottom: 40px; }
          h1 { font-size: 32px; font-weight: 900; margin: 0; line-height: 1; letter-spacing: -0.02em; }
          .meta { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em; color: #666; margin-bottom: 4px; }
          
          .preview-box { border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden; margin-bottom: 40px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }
          .preview-box img { width: 100%; height: auto; display: block; }
          
          .section { margin-bottom: 40px; page-break-inside: avoid; }
          h2 { font-size: 14px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.05em; border-left: 4px solid #4b2bee; padding-left: 12px; margin-bottom: 20px; color: #000; }
          
          .modules-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
          .module-card { background: #f3f4f6; border: 1px solid #e5e7eb; padding: 10px 14px; border-radius: 8px; font-size: 11px; font-weight: 600; display: flex; align-items: center; gap: 8px; color: #374151; }
          .icon { font-family: 'Material Symbols Outlined'; font-size: 16px; color: #4b2bee; }
          
          .objective-box { background: #f9fafb; padding: 20px; border-radius: 8px; border: 1px solid #e5e7eb; font-size: 13px; line-height: 1.6; color: #4b5563; }

          .code-block { background: #111827; color: #e5e7eb; padding: 20px; border-radius: 12px; font-family: 'Fira Code', monospace; font-size: 10px; line-height: 1.5; white-space: pre-wrap; overflow-x: hidden; border: 1px solid #374151; }
          
          .footer { margin-top: 60px; padding-top: 20px; border-top: 1px solid #e5e7eb; display: flex; justify-content: space-between; font-size: 9px; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.05em; }
        </style>
      </head>
      <body>
        <div class="header">
            <div>
                <div class="meta">Hero45 Generated Report</div>
                <h1>${project.title}</h1>
            </div>
            <div style="text-align: right;">
                 <div class="meta">Date</div>
                 <div style="font-weight: 700; font-size: 12px;">${new Date().toLocaleDateString()}</div>
            </div>
        </div>
        ...
        <div class="section">
           <h2>Visual DNA & Architecture</h2>
           <div style="margin-bottom: 15px; font-size: 12px; font-weight: 600; color: #4b5563;">Style: <span style="color: #000; text-transform: uppercase;">${project.visualStyle}</span></div>
        ...
        <div class="section">
           <h2>Base44 Technical Spec</h2>
           <div class="code-block">${techSpec || "// Technical specifications not generated yet."}</div>
        </div>
        
        <div class="footer">
            <span>Project ID: ${project.id}</span>
            <span>Generated by Hero45 AI</span>
        </div>
    ...
    // Open in new window/tab - Safe & Reliable
